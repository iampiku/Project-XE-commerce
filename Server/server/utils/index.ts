...
/** Authorization Check for Seller or Customer - True/False */
export const requiresToBeSeller = async function (req: RequestInterface, res: ResponseInterface, next: Next) {
    try {
        const {userId, roleId} = req as any;
        return Role.findByPk(roleId).then((rolePresent) => {
            if (rolePresent && rolePresent.getDataValue('role') === 'SELLER') {
                return next();
            }
            return next(warn(res, FORBIDDEN, 'Please register yourself as a seller first!'));
        })

    } catch (e) {
        return next(warn(res, BAD_REQUEST, e || 'You are not a seller! Not Authorized!'))
    }
}
/* Auhtorized and Important check The
*  currentUser must be an ADMIN Rolled */
export const requiresToBeAdmin = async function (req: RequestInterface, res: ResponseInterface, next: Next) {
    try {
        // TODO: Please Implement the method correctly!
        // For the sake of purpose I mirrored the functionality!
        return next();
    } catch (e) {
        return next(warn(res, BAD_REQUEST, e || 'You are not an ADMIN! Not Authorized!'));
    }
}
