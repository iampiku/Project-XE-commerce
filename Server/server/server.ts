...
export class Server {
  ...
  private async internalServerStart() {
    try {
      ...
      await (async () => {
         ...
         */
        await this.initializeRolesForRoleBasedAuthentication();
        await this.createAdminUserIfNotExists();
      })();

      this.app.use("/api", APIController); // api/v1 , /api/*
  ...
  private async initializeRolesForRoleBasedAuthentication() {
    await Role.findOne({ where: { role: RoleInterface.CUSTOMER } }).then(
      (m) => {
        if (m == null) {
          (async function () {
  ...
  private async createAdminUserIfNotExists() {
    ...
    try {
      await Role.findOne({ where: { role: RoleInterface.ADMIN } }).then(
        async (m) => {
          if (m !== null) {
