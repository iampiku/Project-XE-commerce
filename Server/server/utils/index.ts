...
export const requiresToBeSeller = async function (
  req: RequestInterface,
  res: ResponseInterface,
  next: Next
) {
  try {
    ...
    return Role.findByPk(roleId).then((rolePresent) => {
      if (rolePresent && rolePresent.getDataValue("role") === "SELLER") {
        return next(); // sellerId -> httpHeader -> productProductController
      }
      return next(
        warn(res, FORBIDDEN, "Please register yourself as a seller first!")
      );
    });
  } catch (e) {
    return next(
      warn(res, BAD_REQUEST, e || "You are not a seller! Not Authorized!")
    );
  }
};
/* Auhtorized and Important check The
 *  currentUser must be an ADMIN Rolled */
export const requiresToBeAdmin = async function (
  req: RequestInterface,
  res: ResponseInterface,
  next: Next
) {
  try {
    ...
    return next(
      warn(res, BAD_REQUEST, e || "You are not an ADMIN! Not Authorized!")
    );
  }
};
