...
const Dashboard: React.FC<DashboardProps> = ({}: DashboardProps) => {
    ...
    function Card(): JSX.Element {
        return (
            <React.Fragment>
                <div
                    className={'bg-white p-2 hover:shadow-xl hover:bg-gray-50 cursor-pointer rounded-md border border-blue-600 shadow'}>
        ...
        <React.Fragment>
            ...
            <div className="my-4 p-6">
                <div className="flex items-center max-w-full justify-center space-x-3 overflow-x-auto">
                    {[1, 2, 3, 4, 5].map((_, index: number) => <Card key={index}/>)}
                </div>

                <div className="my-3">
                    {categories.map((category: any) => <div
                        key={category.id}> {category.name} - {category.description} </div>)}
                </div>
            </div>
        </React.Fragment>
    );
