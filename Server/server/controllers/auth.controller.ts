...
router.post(
  ...
  async (req: RequestInterface, res: ResponseInterface, next: Next) => {
    try {
      ...
      const sellerRoleInstance = await Role.findOne({
        where: { role: RoleInterface.SELLER },
      });
      ...
      return res
        .status(OK)
        .send({ ...SUCCESS, message: `user has been upgraded to a seller!` });
    } catch (e) {
      return warn(
        res,
        FORBIDDEN,
        e || "You are not authorized as Admin Profile"
      );
    }
    next();
  }
